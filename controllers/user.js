...
exports.userSignIn = async (req, res) => {
  ...
  if (!user)
    res.json({ success: false, message: "User not found, with given email!" });

  const isMatch = await user.comparePassword(password);
  if (!isMatch)
    res.json({ success: false, message: "Email / Password is not match" });
...
exports.uploadProfile = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "server error, try afeter sometime" });
